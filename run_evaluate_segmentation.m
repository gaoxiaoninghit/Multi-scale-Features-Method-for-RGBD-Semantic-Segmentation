...
for stage = 1 : params.seg.numStages
  ...
  for ii = 1 : consts.numImages
    fprintf('Evaluating segmentation %d/%d (stage %d)\r', ii, consts.numImages, stage);
    ...
    %%imgInstanceLabels中将同一类但是不同物体进行了区分，imgObejct并没有区分，
    %是不是imgInstanceLabels将我们定义的40类计算了进去，这个有待考证
    ...
    %此处的imgRegionsTrue就是含有不同的Mask的（第几个Mask）
